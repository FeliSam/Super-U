...
export const API_BASE_OVERRIDE_KEY = 'marche-dore.apiBase.v1';
...
/** Candidates ordered: override → env/tunnel → LAN:8787 → packager → loopback (web). */
export function listApiBaseCandidates(): string[] {
  ...
  // Ne pas prioriser le LAN si le build pointe déjà un tunnel / API publique
...
export async function loadApiBaseOverride(): Promise<string | null> {
  try {
    if (typeof localStorage !== 'undefined') {
      ...
      if (web) {
        ...
        void appStorage.setItem(API_BASE_OVERRIDE_KEY, memoryOverride).catch(() => undefined);
        emit();
        return memoryOverride;
      }
    }
  ...
  try {
    const stored = await appStorage.getItem(API_BASE_OVERRIDE_KEY);
    if (stored) {
...
export async function persistApiBaseOverride(url: string | null): Promise<string> {
  ...
  try {
    if (next) await appStorage.setItem(API_BASE_OVERRIDE_KEY, next);
    else await appStorage.removeItem(API_BASE_OVERRIDE_KEY);
  } catch {
    /* ignore */
  }
  emit();
