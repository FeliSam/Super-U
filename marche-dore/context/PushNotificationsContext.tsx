...
const TOKEN_KEY = 'marche-dore.pushToken.v1';
...
export function PushNotificationsProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      setReady(false);
      // Expo Go (SDK 53+) : push distant retiré — pas d’import notifications.
      ...
      if (!session || !pushEnabled) {
        if (!pushEnabled) {
          const platform = Platform.OS === 'ios' ? 'ios' : 'android';
          await registerCommsDevice(platform, null).catch(() => undefined);
        }
        if (mounted) setReady(true);
        return;
      }
      ...
      try {
        ...
        /* missing native module / projectId */
      } finally {
        if (mounted) setReady(true);
      }
    };
  ...
  }, [session, pushEnabled, registerDevice]);
