...
export function sortBySlot<T extends { slot_label?: string | null; slot_id?: string | null; created_at?: string }>(
  items: T[],
): T[] {
  return [...items].sort((a, b) => {
    const ra = slotKindRank(slotKind(a.slot_id, a.slot_label));
    const rb = slotKindRank(slotKind(b.slot_id, b.slot_label));
