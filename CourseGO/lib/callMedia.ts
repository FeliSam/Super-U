...
function partial(
  ...
  amp.gain.exponentialRampToValueAtTime(Math.max(0.0002, gain * 0.38), time + Math.min(0.07, dur * 0.25));
...
function chime(ctx: AudioContext, dest: AudioNode, time: number, freq: number, dur: number, gain: number) {
  partial(ctx, dest, time, freq, dur, gain, 'triangle', 4600);
  partial(ctx, dest, time, freq, dur, gain * 0.55, 'sine', 3400);
  partial(ctx, dest, time, freq * 2.01, dur * 0.48, gain * 0.16, 'sine', 7200);
}
...
function staffIncoming(ctx: AudioContext, dest: AudioNode, t0: number) {
  chime(ctx, dest, t0, 440.0, 0.12, 0.13);
  chime(ctx, dest, t0, 659.25, 0.12, 0.1);
  chime(ctx, dest, t0 + 0.14, 523.25, 0.12, 0.14);
  chime(ctx, dest, t0 + 0.28, 659.25, 0.12, 0.14);
  chime(ctx, dest, t0 + 0.42, 880.0, 0.26, 0.16);
  chime(ctx, dest, t0 + 0.74, 659.25, 0.11, 0.12);
  chime(ctx, dest, t0 + 0.88, 440.0, 0.28, 0.11);
  chime(ctx, dest, t0 + 0.88, 659.25, 0.28, 0.08);
}

function staffOutgoing(ctx: AudioContext, dest: AudioNode, t0: number) {
  chime(ctx, dest, t0, 440.0, 1.05, 0.11);
  chime(ctx, dest, t0, 480.0, 1.05, 0.1);
  chime(ctx, dest, t0 + 1.28, 440.0, 1.05, 0.11);
  chime(ctx, dest, t0 + 1.28, 480.0, 1.05, 0.1);
}
...
export function startRingtone(kind: 'in' | 'out') {
  ...
  const burst = () => {
    ...
    if (kind === 'in') staffIncoming(ctx, bus.master, t0);
    else staffOutgoing(ctx, bus.master, t0);
  };
  burst();
  const iv = setInterval(burst, kind === 'in' ? 2400 : 4200);
  ring = {
    ctx,
    stop: () => {
      live = false;
